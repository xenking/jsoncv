// Map hostnames to resume names
const RESUME_MAP = {
  'cv.oliviadilan.com': 'Anna Tishchenko CV',
};
...
export default {
  async fetch(request, env) {
    ...
    console.log('Request:', url.hostname, pathname);

    // Serve the resume at root path
    if (pathname === '/' || pathname === '') {
      const resumeName = RESUME_MAP[url.hostname] || env.RESUME_NAME || 'Richard Hendriks CV';
